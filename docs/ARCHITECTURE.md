...
| Autoplot user summary | `autoplot/summary.py` | Read-only projection of persisted evidence, publish integrity, delivery state, and the completed FigurePlan; it does not reparse or rehash the plan. |
| Read-only FigurePlan preview | `plan_preview.py` | Builds one v1 planned/not-applicable/blocked payload from the existing semantic, Study Model, and FigurePlan owners. It never renders, materializes terminal data, writes a project, or runs completed-plan gates. |
