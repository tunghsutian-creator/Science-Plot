...
执行内核和 provider-neutral 请求生命周期：统一
...
工作区显示；响应绑定完整请求哈希，取消后的迟到结果会被丢弃。数据映射现在支持零写入
预览、精确哈希确认、封闭变换、原始数据不变、原子执行、
标准 Studio 项目、执行清单防篡改、确定性输出复算、样本覆盖门禁和完整交付。当前尚未
接入生产模型 adapter，也尚未从 Canvas 确认卡执行映射，因此 M3 仍处于开发中。
...
再由独立确认收据绑定提案 SHA、请求 SHA 和每个源文件 SHA，最后由程序执行：
...
- 注入 provider 后显示真实请求输入、共享上下文说明、就地进度和 Stop；
...
- M2 选择驱动的十类对象检查器、数据点选择、原生 label 拖动、结构 QA、五工具审阅层、
  五类锚点和四类原生标注晋升；
- M3 provider-neutral Assistant 面板、`CanvasSession` v5 事务状态机、哈希绑定请求记录、
  线程化进度/取消、零修改预览、实时
  apply、暂停/拒绝/逐批撤销/提交/整轮回滚、冲突与中断恢复、幂等日志 outbox；
- M3 `DataMappingProposal` v2、外部确认收据、零写预览、封闭确定性变换、原子候选项目、
  transform ledger、映射样本覆盖门禁，以及 run/Studio/QA/delivery 接入；
