...
2. Inspect local materials rules before inventing extraction logic:
   ...
   ```
3. Choose the route:
   - Human file grouping: use `sciplot prepare PATH --out outputs/intake_projects --json`, then `sciplot intake PATH --out outputs/intake_projects` to open a prefilled UI and run the exported `plot_request.json`.
   - Repeatable figure request: use `sciplot run REQUEST.json`.
   - Unknown mixed experiment request: use `sciplot run` with `"recipe": "auto"`.
   - Torque-rheometer curation: use `sciplot curate torque PATH --name PROJECT_NAME --out outputs/curation_projects --json`, review `curation/torque_review.html`, then run the exported `plot_request.json`.
...
# Open the local file-grouping Web UI
