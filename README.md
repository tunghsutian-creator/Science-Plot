...
安全边界与验收证据见
[docs/SCIPLOT_DATA_MAPPING_M3_AUDIT.md](docs/SCIPLOT_DATA_MAPPING_M3_AUDIT.md)。
...
当前已完成 M0/M1、M2 的工程内核，以及 M3 的可逆 Assistant 事务内核和确定性数据映射
执行内核：统一
...
可用。数据映射现在支持零写入预览、精确哈希确认、封闭变换、原始数据不变、原子执行、
标准 Studio 项目、执行清单防篡改、确定性输出复算、样本覆盖门禁和完整交付。当前尚未
接入真实模型，也尚未把映射确认流接入 Canvas Assistant UI，因此 M3 仍处于开发中。
