...
正式渲染器和默认日常前端都是 Veusz。SciPlot 直接保留原生 `MainWindow` 的对象树、属性
编辑器、Datasets、菜单、快捷键和 Undo/Redo，并以同一个 Veusz `Document` 和 exact-current
VSZ 为视觉权威。新增能力只通过 `SciPlot` 菜单、按需显示且可关闭的 dock，以及轻量状态提示
接入；不替换、重排或复制 Veusz 的核心交互，也不开发第二个渲染器或第二套文档/撤销模型。

完整阶段、架构边界和验收门见
[DEVELOPMENT_ROADMAP.md](DEVELOPMENT_ROADMAP.md)。Veusz 集成的操作流、窗口结构和视觉原则见
...
## 产品方向：Veusz 原生界面上的 AI-enhanced SciPlot

未来的日常工作台遵循三个分工：

- 用户继续用 Veusz 原生对象树、属性面板、Datasets、画布、菜单和快捷键完成日常编辑；
- AI 把自然语言、当前 Veusz 选择和 QA 上下文转换为经过验证的类型化操作；
- 确定性程序在同一个 `Document` 上执行数据变换、文档修改、Undo/Redo、QA、导出和交付。
...
独立确认收据和 transform ledger 已经建立。原生 Qt Canvas 原型直接嵌入 `PlotWindow`，
验证了选择、编辑、撤销/重做、保存、恢复、精确导出、QA 和项目交付等能力。它的实现和
探针继续作为能力库与实验入口；其中经过验证且有日常价值的组件将逐步接入 Veusz
`MainWindow`，而不是把该原型晋升为替代前端。

M2 的非导出审阅层也已实现：五种工具、五类坐标锚点、独立 sidecar、关闭重开、审阅到
Veusz 原生标注的类型化晋升，以及撤销/重做和精确导出门禁均已通过。自动探针不冒充真人
验收；早期“完成真实会话后把 `studio` 切换到 Canvas-default”的路线已于 2026-07-18
superseded，`studio` 保持 Veusz 原生界面。

在独立 Canvas 原型中，M3 Assistant 已实现为 Inspector 的第三个自适应工作区。
...
完成，因此 M3 仍处于开发中。当前路线会把这套事务和请求界面提取为 Veusz
`MainWindow` 的可关闭 dock，而不是继续扩张独立 Inspector 外壳。
...
会话仍未完成；M3 的真实生产模型六项自然语言任务也仍需实测，不能用内存
...
不凑正式指标，冻结后任何运行时代码变化都会重新计数。候选必须已经包含最终的
Veusz-first 集成形态；只有用户审阅证据并明确批准后，才把同一提交/同一包原样晋升到正常
工作区并运行可回滚 canary，不在验收后另改入口或清理代码。早期候选要求的
Canvas-default 切换已 superseded；Veusz `MainWindow` 始终是日常编辑入口。M7 分发不在
当前目标内。
...
## 原生 Canvas（保留的实验入口与能力库）

以下命令保留用于回归、实验和提取已验证组件，不是默认日常前端：

```bash
skill/scripts/sciplot canvas PROJECT_OR_VSZ
```

实验 Canvas 不需要 AI。若要在同一窗口启用 OpenAI Assistant，只在 shell 环境提供密钥；
不要把密钥写进仓库、请求文件或项目目录：
...
提供显式项目意图。下列清单记录独立 Canvas 原型已经验证的能力；其中 AI 事务、Review、
QA、来源审计和类型化操作是后续 Veusz 集成的候选组件，并不意味着继续建设替代
`MainWindow` 的产品外壳：
...
SciPlot 不复制 Veusz 的完整属性树。日常对象和属性操作继续由原生 Veusz 完成；SciPlot
只接入自身独有的 AI、QA、来源与交付能力。`canvas` 继续用于组件回归和受控实验，不再
承担默认前端切换目标。

## Veusz 原生日常编辑

需要打开或继续编辑 exact-current VSZ 时，运行交付包内的
`delivery/Open_in_Veusz.command`，或运行：
...
该入口是默认日常编辑路径，不会在 Canvas 验收后退为隐藏或恢复入口。后续 AI、Review、
QA 和来源能力都在同一个 Veusz `MainWindow` 中按需接入。
...
- `src/sciplot_gui/`：实验 Canvas、可提取的 AI/Review/QA 组件、Composition Board、
  controller 和原生 Veusz composition compiler；它不承担默认日常外壳；
- `src/sciplot_core/_vendor/`：迁移兼容层，默认不直接修改；
- `third_party/veusz/`：固定版本的上游生产渲染器与默认日常 `MainWindow`。
