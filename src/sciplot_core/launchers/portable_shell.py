"""Build portable SciPlot and Veusz shell-discovery fragments."""
...
from sciplot_core._paths import PACKAGE_ROOT, REPO_ROOT
...
def portable_sciplot_prelude(*, directory_var: str = "PROJECT_DIR") -> list[str]:
    ...
    source_root = (
        Path(os.environ.get("SCIPLOT_SOURCE_ROOT") or PACKAGE_ROOT.parent)
        .expanduser()
        .resolve()
    )
    python_candidates = [
...
def portable_vsz_finder(
    *,
    ...
    candidates.extend(
        shlex.quote(str(Path(item).expanduser())) for item in extra_candidates
    )
    lines = [
        "",
        "find_vsz() {",
