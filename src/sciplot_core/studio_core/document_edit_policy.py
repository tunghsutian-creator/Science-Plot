...
def _is_line_color(setting_path: Any) -> bool:
    return isinstance(setting_path, str) and setting_path.endswith("/PlotLine/color")
        ...
        path: {
            **widget,
            "editable_fields": [
                ...
                if not _is_line_color(field.get("setting_path"))
                or (
                    widget.get("type") == "xy" and path in allowed
                    and field["setting_path"] == f"{path}/PlotLine/color"
                )
            ],
        }
...
def validate_edit_science_policy(
    ...
    for change in changes:
        ...
        if _is_line_color(setting) and (
            not isinstance(path, str) or path not in allowed
            or setting != f"{path}/PlotLine/color"
        ):
            raise ValueError(
                "Line color editing requires a uniquely bound ordinary curve. "
