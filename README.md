...
fixture hash；这些来源强度差异会保留显示。M5 的 AI 成功晋升/学习闭环以及 M6 的真人日常
会话和默认入口切换仍未完成；M3 的真实生产模型六项自然语言任务也仍需实测，不能用内存
wire fixture 冒充。正式路线先建立会话证据合同，再完成 M5a 晋升机制和 M3
...
E0 会话证据合同现已落为 `sciplot sessions`。正式 M3/M6 轮次必须在操作前把自然任务、
...
transform ledger 必须以该确认步骤开头并以实际绘图数据快照结束。
